...
public class PageView {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<String> inputStream = env.readTextFile("D:\\tencent\\workspace\\FlinkTest\\NetworkFlowAnalysis\\src\\main\\resources\\UserBehavior.csv");
        ...
        SingleOutputStreamOperator<PageViewCount> pvResultStream1 = pvStream.keyBy(data -> data.getWindowEnd())
                .process(new TotalPvCount());
        pvResultStream.print();
        pvResultStream1.print("result1");
        env.execute();
    ...
    private static class TotalPvCount extends KeyedProcessFunction<Long, PageViewCount, PageViewCount> {
        ...
        public void open(Configuration parameters) throws Exception {
            valueState = getRuntimeContext().getState(new ValueStateDescriptor<Long>("total-count", Long.class, 0L));
        }

        @Override
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<PageViewCount> out) throws Exception {
            Long total = valueState.value();
            out.collect(new PageViewCount("pv", ctx.getCurrentKey(), total));
        }
